#!/usr/bin/env bash
# deploy-otel.sh — Deploy the production OpenTelemetry Collector stack (nanovms)
...
# Select compose file: pass --production to use the production stack.
...
# ── Pull images ────────────────────────────────────────────────────
echo "[nanovms-otel] Pulling images …"
docker compose -f "${COMPOSE_FILE}" pull --quiet

# ── Start stack ────────────────────────────────────────────────────
echo "[nanovms-otel] Starting collector stack in background …"
...
echo "[nanovms-otel] Waiting for collector health (timeout ${TIMEOUT}s) …"
while (( ELAPSED < TIMEOUT )); do
    if curl -sf "${HEALTH_URL}" &>/dev/null; then
        echo "[nanovms-otel] Collector is healthy."
        break
    fi
    sleep "${INTERVAL}"
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  nanovms OpenTelemetry Collector — Endpoints"
echo "═══════════════════════════════════════════════════════════"
...
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "[nanovms-otel] Deployment complete."
