...
export default function SprintTracker({ onDataChange }: Props) {
  ...
  const handleCreate = () => {
    ...
    createSprint(newName.trim(), newStart, newEnd, newGoal.trim());
    setNewName("");
    setNewStart("");
    setNewEnd("");
    setNewGoal("");
  ...
  const handleActivate = (id: string) => {
    ...
    if (activeSprint && activeSprint.id !== id) {
      updateSprint(activeSprint.id, { status: "completed" });
    }
  ...
  const handleVelocityChange = (id: string, velocity: number) => {
    updateSprint(id, { velocity });
    refresh();
  };

  // Compute velocity chart data (completed sprints only)
        ...
        <div className="sprint__current">
          ...
          <div className="sprint__progress">
            <div className="sprint__progress-bar">
              <div
                className="sprint__progress-fill"
          ...
          <div className="sprint__velocity-bars">
            {velocityData.map((v, i) => (
              <div key={i} className="sprint__velocity-bar">
                ...
                <div
                  className="sprint__velocity-bar-fill"
                  style={{ height: `${(v.velocity / maxVelocity) * 100}%` }}
                />
              ...
              <div className="sprint__item-info">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleVelocityChange(sprint.id, parseInt(e.target.value) || 0)
                    }
